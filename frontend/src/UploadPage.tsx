...
const UploadPage: React.FC<UploadPageProps> = ({ onUploadSuccess }) => {
    ...
    return (
        <div className="bg-gradient-to-br from-gray-900 to-gray-700 min-h-screen flex items-center justify-center p-4 font-inter">
            <div className="bg-gray-800 rounded-xl shadow-2xl p-6 md:p-8 w-full max-w-2xl text-center border border-gray-700">
                <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-8">Upload Budget Data</h1>
                ...
                <div className="flex flex-col md:flex-row justify-center gap-6 mb-8">
                    {/* Spending Upload */}
                    <div className="flex flex-col items-center p-4 bg-gray-700 rounded-lg shadow-md border border-gray-600 w-full md:w-1/2">
                        <h2 className="text-2xl font-bold text-blue-300 mb-4">Spending Data</h2>
                        <input
                            type="file"
                        ...
                        <button
                            ...
                            className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-full transition-colors duration-300 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={uploadingSpending}
                        >
                            {uploadingSpending ? 'Uploading...' : 'Upload Spending Excel'}
                        </button>
                        {spendingMessage && <p className="mt-2 text-sm text-blue-300">{spendingMessage}</p>}
                    </div>

                    {/* Income Upload */}
                    <div className="flex flex-col items-center p-4 bg-gray-700 rounded-lg shadow-md border border-gray-600 w-full md:w-1/2">
                        <h2 className="text-2xl font-bold text-green-300 mb-4">Income Data</h2>
                        <input
                            type="file"
                        ...
                        <button
                            ...
                            className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-full transition-colors duration-300 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={uploadingIncome}
                        >
                            {uploadingIncome ? 'Uploading...' : 'Upload Income Excel'}
                        </button>
                        {incomeMessage && <p className="mt-2 text-sm text-green-300">{incomeMessage}</p>}
                    </div>
                </div>
                ...
                {(spendingMessage || incomeMessage) && !uploadingSpending && !uploadingIncome && (
                    <button
                        ...
                        className="mt-8 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-full transition-colors duration-300 shadow-xl text-lg"
                    >
